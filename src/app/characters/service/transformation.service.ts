...
export class TransformationService {
  private transformationsSubject = new BehaviorSubject<any[]>([
    {
      "id": 1,
      "idp":1,
      "name": "Goku SSJ",
  ...
  transformations$ = this.transformationsSubject.asObservable(); 
  private selectedTransformation: any = null;
  ...
  updateTransformation(updatedTransformation: any): void {
    const transformations = this.transformationsSubject.getValue();
    const index = transformations.findIndex(trans => trans.id === updatedTransformation.id);
    if (index !== -1) {
      transformations[index] = updatedTransformation;  // Actualizamos el `ki` modificado
      this.transformationsSubject.next([...transformations]);  // Emitimos los cambios
    }
  }
}
